...
def log_validation(vae, text_encoder, tokenizer, unet, controlnet, args, accelerator, weight_dtype, step):
    ...
    for validation_prompt, validation_image in zip(validation_prompts, validation_images):
        validation_image = Image.open(validation_image).convert("RGB").resize((512, 512))
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                timesteps = torch.randint(args.min_train_timesteps, args.max_train_timesteps, (bsz,), device=latents.device)
                ...
                if args.controlnet_model_name_or_path == "lllyasviel/control_v11p_sd15_canny":
                    ...
                    with torch.no_grad():
                        if args.controlnet_model_name_or_path == "lllyasviel/control_v11p_sd15_softedge":
                            reward_model = get_reward_model('hed', 'https://huggingface.co/lllyasviel/Annotators/resolve/main/ControlNetHED.pth')
                            reward_model.to(accelerator.device)
                            reward_model.eval()
                        elif args.controlnet_model_name_or_path == "lllyasviel/control_v11p_sd15_lineart":
                            reward_model = get_reward_model('lineart', 'https://huggingface.co/spaces/awacke1/Image-to-Line-Drawings/resolve/main/model.pth')
                            reward_model.to(accelerator.device)
                            reward_model.eval()
                        else:
                        ...
                        denormalized_condition_image = controlnet_image * std + mean
                        with torch.no_grad():
                            controlnet_image = reward_model(denormalized_condition_image)
                        controlnet_image = controlnet_image.expand(-1, 3, -1, -1)  # (B, 3, H, W)

                        if args.controlnet_model_name_or_path == "lllyasviel/control_v11p_sd15_lineart":
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        # FSDP save model need to call all the ranks, instead of the main process
                ...
                logs = {"loss_step": train_loss, "lr": lr_scheduler.get_last_lr()[0]}
        ...
        logs = {"loss_epoch": loss_per_epoch / len(train_dataloader)}
        progress_bar.set_postfix(**logs)
